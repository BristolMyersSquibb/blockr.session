...
blockr.session adds project (i.e. board) persistence to [blockr](https://bristolmyerssquibb.github.io/blockr.core/) apps: save, restore and manage boards from within a running app.
It provides a `manage_project()` plugin backed by the [pins](https://pins.rstudio.com/) package -- a more capable alternative to blockr.core's built-in `preserve_board()`, which offers only simple file upload and download.
