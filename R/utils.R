...
# Package utilities
pkg_name <- function() "blockr.session"
...
coal <- function(...) {
  for (x in list(...)) {
    if (!is.null(x) && length(x) > 0 && !identical(x, "")) return(x)
  }
  NULL
}
...
#' Uses `pins::pin_search()` for efficient bulk metadata retrieval.
#'
#' @param backend A pins board (default: from blockr_option)
#' @return Data frame with columns: name, created, time_ago
#'
#' @export
list_workflows <- function(backend = blockr_option("session_mgmt_backend",
                                                    pins::board_local())) {
  df <- tryCatch(
